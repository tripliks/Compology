...
void setup() {
        ...
        Serial.println("Begin ultrasonic I2C test");
}
...
void loop() {
  ...
  for (byte sampleNum = 0; sampleNum < numberSamples; sampleNum++)
  {
    ...
    bigBuffer[sampleNum] = range;
    Serial.println(bigBuffer[sampleNum]);
  }
  
//  Serial.print("Last sample: "); Serial.println(bigBuffer[numberSamples-1]);
  sendData += bigBuffer[numberSamples-1];
  Serial.println(sendData);
  delay(2000);

  const byte id = 2;
  int us = bigBuffer[numberSamples-1];
  int ir = 121;
  int t1 = 20;
  int h1 = 20;
  int t2 = 25;
